  ...
  const errorMessage = (() => {
    switch (true) {
      case !isEndpointActive:
        return pt_BR.sessions.endpointNotConnected
      case !selectedEndpoint:
        return pt_BR.sessions.selectEndpoint
      case !agentId:
        return pt_BR.sessions.selectAgent
      case !hasStorage:
        return (
          <>
            {pt_BR.sessions.connectStorage}{' '}
            <Link
              className="underline"
              href={'https://docs.agno.com/storage'}
              target="_blank"
            >
              {pt_BR.sessions.storageLink}
            </Link>{' '}
            {pt_BR.sessions.toYourAgent}
          </>
        )
      default:
        return pt_BR.sessions.noSessionRecords
    }
  })()
