  ...
  const handleDeleteSession = async () => {
    ...
    try {
      ...
      if (response?.ok && sessionsData) {
        ...
        toast.success(pt_BR.sessions.deleteSuccess)
      } else {
        const errorMsg = await response?.text()
        toast.error(
          `${pt_BR.sessions.deleteFailed}: ${response?.statusText || 'Unknown error'} ${errorMsg || ''}`
        )
      }
    } catch (error) {
      toast.error(
        `${pt_BR.sessions.deleteFailed}: ${error instanceof Error ? error.message : String(error)}`
      )
    } finally {
