...
const NewChatButton = ({
  ...
  >
    ...
    <span className="uppercase">{pt_BR.sidebar.newChat}</span>
  </Button>
)
...
const Endpoint = () => {
  ...
  const handleSave = async () => {
    if (!isValidUrl(endpointValue)) {
      toast.error(pt_BR.messages.invalidUrl)
      return
    }
              ...
              {isHovering ? (
                ...
                >
                  <p className="text-primary flex items-center gap-2 whitespace-nowrap text-xs font-medium">
                    <Icon type="edit" size="xxs" /> {pt_BR.sidebar.editEndpoint}
                  </p>
                </motion.div>
              ) : (
                <motion.div
  ...
  return (
    ...
    >
      <motion.button
        ...
        aria-label={
          isCollapsed
            ? pt_BR.sidebar.expandTooltip
            : pt_BR.sidebar.collapseTooltip
        }
        type="button"
        whileTap={{ scale: 0.95 }}
      >
